        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            if request.user.is_authenticated:
                try:
                    ...
                    if product_status:
                        if Cart.objects.filter(user=request.user.id,product_id=pid):
                            return JsonResponse({'status':'Product Already in cart'}, status=200)
                        else:
                            if product_status.quantity>=product_qty:
                                Cart.objects.create(user=request.user,product_id=pid,product_qty=product_qty)
                ...
                return JsonResponse({'status': 'Please log in to add to cart'}, status=401)     
        else:
            return JsonResponse({'status': 'Invalid Access (non-AJAX)'}, status=400)

    return JsonResponse({'status': 'Invalid request method, POST expected'}, status=405)
...
def fav_page(request):
    if request.headers.get('x-requested-with')=='XMLHttpRequest':
        if request.user.is_authenticated:
            ...
            if product_status:
                if Favourite.objects.create(user=request.user,product_id=product_id):
...
def login_page(request): 
    if request.user.is_authenticated:
      ...
      if request.method=='POST':
        ...
        print(f"Username entered: {name}")  # Debug
